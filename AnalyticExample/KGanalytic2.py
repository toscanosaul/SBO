...
lowerX=-3.0
upperX=3.0
varianceW1=np.array(10e12).reshape(1)
muW1=np.array(0.0).reshape(1)

varianceW2=np.array(1.0).reshape(1)
...
def noisyG(X,n):
    ...
    if len(X.shape)==2:
       X=X[0,:]
    W=np.random.normal(0,varianceW1,n).reshape((n,n2))
    
    res=np.zeros(n)
    for i in xrange(n):
        t=np.array(np.random.normal(-1.0,varianceW2,1))
        res[i]=g(X,W[i,:],t)
        ##alphad/n
...
def simulatorW(n):
    ...
    return np.random.normal(0,varianceW1,n).reshape((n,n2))
