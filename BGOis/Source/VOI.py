...
class VOISBO(VOI):
    ...
    def aANDb(self,n,x,xNew,wNew,L,temp2,past,kerns,B):
        """
        Output: A tuple with:
        ...
        BN=np.zeros([m,1])
        n2=self.n2
        BN[:,0]=B(x,np.concatenate((xNew,wNew)),self.n1,n2,kerns) #B(x,n+1)
        n1=self.n1
        n2=self.n2
        new=np.concatenate((xNew,wNew)).reshape((1,n1+n2))
        xNew=xNew.reshape((1,n1))
    ...
    def evalVOI(self,n,pointNew,a,b,c,keep,keep1,M,gamma,BN,L,inv,aux4,kerns,XW,
        ...
        gradXSigma0=self._gradXWSigmaOfunc(n,pointNew,
                                                       kerns,XW[0:tempN,0:n1],
                                                       XW[0:tempN,n1:n1+n2],
                                                       n1,n2,nTraining,gamma)

        gradXB=self._gradXBfunc(pointNew,kerns,BN,keep,self._points,n1,n2)
#        gradWB=self._gradWBfunc(pointNew,kern,BN,keep,self._points)

    
        gradientGamma=gradXSigma0.transpose()
        
        IS=int(pointNew[0,n1])
        kern=kerns[IS]
        inv3=inv
        xNew=pointNew[:,0:n1]

        beta1=(kern.A(xNew)-aux4)
        gradient=np.zeros(M)
        result=np.zeros(n1)
    ...
    def VOIfunc(self,n,pointNew,grad,L,temp2,a,scratch,kerns,XW,B,onlyGradient=False):
        """
        Output:
            Evaluates the VOI and it can compute its derivative. It evaluates
            the VOI, when grad and onlyGradient are False; it evaluates the
