...
class SBOGP(GaussianProcess):
    def __init__(self,B,dimNoiseW,dimPoints,gradXBforAn=None, computeLogProductExpectationsForAn=None,
                 IS=0,SEK=True,mat52=False,*args,**kargs):
        ...
        if SEK:
            ...
            for i in range(self.numberIS):
                ...
                tmp=SK.SEK(self.n1,X=self.data.Xhist[ind,0:self.n1],
    ...
    def computeA(self,X,noise):
        n=X.shape[0]
        A=np.zeros((n,n))
        for i in range(n):
            for j in range(n):
                if X[i,self.n1]==X[j,self.n1]:
                    m=int(X[i,self.n1])
                    a=X[i:i+1,0:self.n1]
                    b=X[j:j+1,0:self.n1]
                    A[i,j]=self._k[m].K(a,b)
                    if i==j:
                        A[i,j]+=noise[i]
        return A
