...
class SBO:
	...
	self.numberIS=self.Obj.numberIS
    ...
    def optimizeVOI(self,start, i,L,temp2,a,B,scratch,IS):
	"""
	    ...
	    z=np.append(x,IS)
            return self.opt.functionGradientAscentVn(z,i=i,VOI=self._VOI,L=L,temp2=temp2,a=a,
    ...
    def getParametersOptVoi(self,i,IS):
	"""
...
#	st=np.concatenate((x1,w1),1)
	args2=self.getParametersOptVoi(i,IS)
	args2['IS']=IS
    #    args2['start']=st
	self.optRuns.append(misc.VOIOptWrapper(self,x1,**args2))
	return self.optRuns[0],self.optRuns[0].fOpt
	#fl.writeNewPointSBO(self,self.optRuns[0])

    def optVOIParal(self,i,nStart,IS,numProcesses=None):
	"""
	...
	It restarts the gradient ascent method nStart times.
	   ...
	   # XWst=np.concatenate((Xst,wSt),1)
	    args3=self.getParametersOptVoi(i,IS)
	    args3['IS']=IS
	    pool = mp.Pool(processes=numProcesses)
	    jobs = []
            for j in range(nStart):
                job = pool.apply_async(misc.VOIOptWrapper, args=(self,Xst[j:j+1,:],),
				       kwds=args3)
                jobs.append(job)
        ...
        if len(self.optRuns):
	    tmpOpt=[o.fOpt for o in self.optRuns]
            j = np.argmax(tmpOpt)
	    return self.optRuns[j],tmpOpt[j]
