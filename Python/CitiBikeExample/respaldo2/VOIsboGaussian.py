...
def Bparameters(x,X,W,n1,n2,variance0,alpha1,alpha2,variance=np.array(1).reshape(1),mu=np.array(0).reshape(1)):
...
##n2 is the dimension of W[i,:]
##lambdaParameter are the parameters of the Poisson processes for W. It can be empty if poisson=False
def update (x,n,y,X,W,xNew,wNew,muStart,kernel,variance0,alpha1,alpha2,n1,n2,varianceObservations):
    if n==0:
        return muStart
    x=np.array(x)
    m=x.shape[0]
    ...
    B=np.zeros([m,n])
    for i in xrange(n):
        B[:,i]=Bparameters(x,X[i,:],W[i,:],n1,n2,variance0,alpha1,alpha2)
    BN=np.zeros([m,1])
    BN[:,0]=Bparameters(x,xNew,wNew,n1,n2,variance0,alpha1,alpha2) #B(x,n+1)
...
def Vn (xNew,wNew,X,W,n,y,muStart,kernel,variance0,alpha1,alpha2,n1,n2,points,varianceObservations,grad=True,mu=np.array(0).reshape(1),variance=np.array(1).reshape(1)):
    if n>1:
        n=n-1
        a,b,gamma,BN,L,B=update(points,n,y,X,W,np.array(xNew).reshape((1,n1)),np.array(wNew).reshape((1,n2)),muStart,kernel,variance0,alpha1,alpha2,n1,n2,varianceObservations)
        ...
        if grad==True:
            if M<=1:
                ...
                for i in xrange(n2):
                    for j in xrange(M):
                    #    temp100=BN[keep[j],0]*norm.expect(lambda x: (x-wNew[0,i])*np.exp(-alpha2[i]*(x-wNew[0,i])**2), loc=0,scale=1)
                    #    temp100=temp100*2.0*alpha2[i]
                    #    temp101=norm.expect(lambda x: np.exp(-alpha2[i]*(x-wNew[0,i])**2), loc=0,scale=1)
        ...
        m=points.shape[0]
        BN=np.zeros([m,1])
        BN[:,0]=Bparameters(points,xNew.reshape(n1),wNew.reshape(n2),n1,n2,variance0,alpha1,alpha2) #B(x,n+1)
        f=open("Bver.txt","w")
        np.savetxt(f,BN)
        f.close()
        b=BN[:,0]
            ...
            if grad==True:
                ...
                for i in xrange(n2):
                    for j in xrange(M):
                       ...
                       # temp101=norm.expect(lambda x: np.exp(-alpha2[i]*(x-wNew[0,i])**2), loc=0,scale=1)
                       # temp100=temp100/temp101
                        temp100=-2.0*alpha2[i]*wNew[0,i]
                        temp101=-alpha2[i]*((mu[i]/variance[i])+2.0*alpha2[i]*wNew[0,i])
                        temp101=temp101/(-alpha2[i]-.5*variance[i])
                        gradWB[j,i]=BN[keep[j],0]*(temp100+temp101)
...
def aN_grad(x,L,y2,X,W,n1,n2,variance0,alpha1,alpha2,muStart,n,gradient=True):
    B=np.zeros(n)
    for i in xrange(n):
        B[i]=Bparameters(x,X[i,:],W[i,:],n1,n2,variance0,alpha1,alpha2)
