...
class SBO:
    def __init__(self,f,alpha1,alpha2,sigma0,sigma,mu,muStart,n1,n2,l,c,d,wStart,muW2,sigmaW2,M,h,h2,mPrior=100,old=False,Xold=-1,Wold=-1,yold=-1):
        ...
        if old==False:
            ...
            self._Xprior=np.random.uniform(c[0],d[0],(mPrior,self._n1)) ##to estimate the kernel
            self._wPrior=self._simulateW1(self._mPrior)
            self._mPrior=mPrior
        else:
            self._X=Xold
            self._W=Wold
    ...
    def B(self,x,X,W):
        ...
        n=10000 ###suggestion: use 100^n where n is the dimension of w1
        samples=self._simulateW1(n)
        tmp10=np.zeros(self._n2)
        result=np.zeros((x.shape[0],n1))
        for i in xrange(x.shape[0]):
            for j in xrange(n1):
                for k in range(self._n2):
                    tmp10[k]=np.mean(np.exp(-self._alpha2[k]*((W[j,k]-samples[i,k])**2)))
            tmp1=self._variance0*np.exp(np.sum(-self._alpha1*((x[i,:]-X[j,:])**2)))
            result[i,j]=np.prod(tmp10)*tmp1
        return result
        ...
        if n>1:
            ...
            if grad==True:
                if M<=1:
                    ...
                    for i in xrange(self._n2):
                        for j in xrange(M):
                            n10=100
                            samples=self._simulateW1(n10)
                            temp100=np.mean(np.exp(-self._alpha2[i]*((wNew[0,i]-samples)**2)))
                            temp101=-2.0*self._alpha2[i]*np.mean((samples-wNew[0,i])*np.exp(-self._alpha2[i]*((wNew[0,i]-samples)**2)))
                            gradWB[j,i]=BN[keep[j],0]*float(temp101)/temp100
                            inv1=linalg.solve_triangular(L,B[keep[j],:].transpose(),lower=True)
            ...
            if M<=1:
                ...
                if grad==True:
                    ...
                    for i in xrange(self._n1):
                        for j in xrange(M):
                            gradXB[j,i]=-2.0*self._alpha2[i]*BN[keep[j],0]*(wNew[0,i]-self._points[keep[j],i])
                        Gradient[i]=-np.dot(np.diff(gradXB[:,i]).T,evalC)
                        
                    for i in xrange(self._n2):
                        for j in xrange(M):
                            gradWB[j,i]=-2.0*self._alpha2[i]*BN[keep[j],0]*(wNew[0,i]-self._points[keep[j],i+self._n1])
    ...
    def updateY (self,n,X,W):
        t=self._simulateW2 (self._M,np.array(W[n,:]))
        t=self._f(X[n,:],W[n,:],t)
        return np.mean(t)
...
if __name__ == '__main__':
    ...
    sigmaW2=np.array([1])
    M=1000
    alg=SBO(f,alpha1,alpha2,sigma0,sigma,mu,muStart,n1,n2,l,a,b,w,muW2,sigmaW2,M,h,h2)
    opt=alg.optimize(7)
    print opt[0]
