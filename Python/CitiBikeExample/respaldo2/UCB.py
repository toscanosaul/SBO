...
class SBO:
    ...
    def steepestAscent (self,n,tol=1e-8,maxit=1000,maxtry=25):
        ...
        for i in xrange(repeat):
            ...
            while tolMet==False:
                ...
                oldEval=g1
                oldPoint=X
                g1,g2=self.UCB(X[0,0:self._n1].reshape((1,self._n1)),self._X,n,self._y,grad=True)
                ...
                def fns(alpha,X_=oldPoint,g2=g2):
                    tmp=X_+alpha*g2
                    x_=tmp[0,0:self._n1]
                    return self.UCB(x_,self._X,n,self._y,grad=False)
                lineSearch=self.goldenSectionLineSearch(fns,tol,maxtry,X,g2)
                X=X+lineSearch*g2
                if max(np.reshape(abs(X-oldPoint),X.shape[1]))<tol or iter > maxit:
                    g1=self.UCB(X[0,0:self._n1].reshape((1,self._n1)),self._X,n,self._y,grad=False)
                    val[i,0]=g1
                    val[i,1:]=X
                    tolMet=True
        MAX=np.argmax(val[:,0])
    ...
    def graphAn (self,n):
        ...
        for i in xrange(m):
            ...
            VOI[i]=self.UCB(points[i],self._X[0:n-1,:],n,self._y[0:n-1,:],False)
            variance[i]=self.varN(points[i],n)
            
        plt.plot(points,VOI)
        plt.xlabel('x',fontsize=26)
        plt.ylabel('VOI',fontsize=24)
        plt.savefig('%d'%n+"informationUCBStandardGaussian.pdf")
     #   plt.title('VOI for n=%d'%n)
      ...
      #  plt.title('$\mu_%d(x)$'%n+' vs G(x)')
        plt.savefig('%d'%n+"UCBStandardGaussian.pdf")
        plt.close("all")
    ...
    def getNPoints(self,n,i1,tol=1e-8,maxit=1000,maxtry=25):
        if self._old==False:
            ...
            f=open('%d'%i1+"G(xn)UCBAnalytic.txt","w")
