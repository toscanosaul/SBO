...
class SBO:
    def __init__(self,f,alpha1,sigma0,sigma,mu,muStart,n1,n2,l,c,d,wStart,muW2,sigmaW2,M,mPrior=5,old=False,Xold=-1,Wold=-1,yold=-1):
    ...
    def EI (self,xNew,X,n,y,grad=False):
        n=n-1
        vec=np.zeros(n)
        for i in xrange(n):
    ...
    def steepestAscent (self,n,tol=1e-8,maxit=1000,maxtry=25):
        ...
        for i in xrange(repeat):
            ...
            while tolMet==False:
                ...
                oldEval=g1
                oldPoint=X
                g1,g2=self.EI(X[0,0:self._n1].reshape((1,self._n1)),self._X,n,self._y,grad=True)
            ...
            #            g1,g2=self.EI(X[0,0:self._n1].reshape((1,self._n1)),self._X,n,self._y,grad=True)
             ...
             #           break
                ...
                def fns(alpha,X_=oldPoint,g2=g2):
                    tmp=X_+alpha*g2
                    x_=tmp[0,0:self._n1]
                    return self.EI(x_,self._X,n,self._y,grad=False)
                lineSearch=self.goldenSectionLineSearch(fns,tol,maxtry,X,g2)
                X=X+lineSearch*g2
                if max(np.reshape(abs(X-oldPoint),X.shape[1]))<tol or iter > maxit:
                    g1=self.EI(X[0,0:self._n1].reshape((1,self._n1)),self._X,n,self._y,grad=False)
                    val[i,0]=g1
                    val[i,1:]=X
                    tolMet=True
        MAX=np.argmax(val[:,0])
    ...
    def graphAn (self,n):
        ...
        for i in xrange(m):
            ...
            VOI[i]=self.EI(points[i],self._X[0:n-1,:],n,self._y[0:n-1,:],False)
            variance[i]=self.varN(points[i],n)
            
        plt.plot(points,VOI)
        plt.xlabel('x',fontsize=26)
        plt.ylabel('VOI',fontsize=24)
        plt.savefig('%d'%n+"EIStandardGaussian.pdf")
    ...
    def getNPoints(self,n,i1,tol=1e-8,maxit=1000,maxtry=25):
        if self._old==False:
            ...
            f=open("2NewResultsEI"+'%d'%self._M+"samples5Prior/"+'%d'%i1+"G(xn)EIAnalytic"+'%d'%self._M+".txt","w")
...
if __name__ == '__main__':
    ...
    for i in range(402,702):
        pool.apply_async(runProgram,args=(i,M,T,))
    pool.close()
    pool.join()
  #  pool=multiprocessing.Pool()
  #  results_async = [pool.apply_async(runProgram,args=(i,M,T,)) for i in range(301,401)]
