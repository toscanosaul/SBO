...
#compute the logarithm of B(x,i). X=X[i,:],W[i,:]. x is a matrix of dimensions nxm where m is the dimension of an element of x.
    ...
    if n>1:
        ...
        if grad==True:
            ...
            if M<=1:
                ...
                for i in xrange(n2):
                    for j in xrange(M):
                        temp200=logSumExp(-alpha2[i]*((expec-wNew[0,i])**2)+expec*log(parameterLamb[j])-logFactorial[expec])
                        temp201=logSumExp(-alpha2[i]*((expec-wNew[0,i])**2)+expec*log(parameterLamb[j])-logFactorial[expec]+log(wNew[0,i]))
                        temp200=-np.exp(temp200)*wNew[0,i]
                        temp201=np.exp(temp201)
                        temp100=temp200+temp201
                  #      temp100=BN[keep[j],0]*poisson.expect(lambda x: (x-wNew[0,i])*np.exp(-alpha2[i]*(x-wNew[0,i])**2), args=(np.sum(lambdaParameter[i]),))
                        temp100=temp100*2.0*alpha2[i]*variance0
                        gradWB[j,i]=temp100
                        inv1=linalg.solve_triangular(L,B[keep[j],:].transpose(),lower=True)
