...
class SBO:
    ...
    def update (self,x,n,y,X,xNew):
        if n==0:
            return self._muStart
        x=np.array(x)
        xNew=xNew.reshape((1,self._n1))
        m=x.shape[0]
        A=self.An(n,X)
        ...
        for j in xrange(m):
            ...
            a[j,0]=self._muStart+np.dot(temp1.T,temp5)
    ...
    def Vn (self,xNew,X,n,y,grad=True):
        if n>1:
            n=n-1
            a,b,L=self.update(self._points,n,y,X,xNew)
            ...
            M=len(keep)
            xNew=np.array(xNew).reshape((1,self._n1))
            if grad==True:
                if M<=1:
                    return 0,0
                else:
                    B=np.zeros((1,n))
                    for i in xrange(n):
                        B[0,i]=self._k.K(xNew,X[i:i+1,:])[:,0]
                    temp2=linalg.solve_triangular(L,B.T,lower=True)
                    temp3=temp2
                    sigmaXnew=sqrt(self._k.K(np.array(xNew).reshape((1,self._n1)))-np.dot(temp2.T,temp3))
                    gradX=np.zeros([M,n])
                    gradX2=np.zeros([n,self._n1])
                    temp=np.zeros([n,self._n1])
                    c=c[keep1+1]
                    c2=np.abs(c[0:M-1])
                    tmp100=norm.pdf(c2)
                    gradient=np.zeros(self._n1)
                    B2=np.zeros((1,n))
                    temp54=np.zeros(M)
                    for j in xrange(self._n1):
                        for i in xrange(M):
                            for p1 in xrange(n):
                                gradX[i,p1]=self._k.K(xNew,X[p1,:].reshape((1,self._n1)))*(2.0*self._alpha1[j]*(xNew[0,j]-X[p1,j]))
            ...
            ###
            for i in xrange(m):
                b[i]=self._k.K(np.array(xNew).reshape((1,self._n1)),self._points[i:i+1,:])[:,0]

            b=b/float(sqrt(self._k.K(np.array(xNew).reshape((1,self._n1)))))
            ...
            if M<=1:
                ...
                if grad==True:
                    ...
                    c2=np.abs(c[0:M-1])
                    tmp100=norm.pdf(c2)
                    sigmaXnew=sqrt(self._k.K(np.array(xNew).reshape((1,self._n1))))
                    gradX=np.zeros(M)
                    gradient=np.zeros(self._n1)
                    B2=np.zeros((1,n))
                    for j in xrange(self._n1):
                        for i in xrange(M):
                            gradX[i]=self._k.K(xNew,self._points[keep[i]:keep[i]+1,:])*(2.0*self._alpha1[j]*(xNew[0,j]-self._points[keep[i],j]))
                        gradX=gradX/(float(sigmaXnew))
                        gradient[j]=-np.dot(np.diff(gradX),tmp100)
                    return h, gradient
                else:
                    return h
    ...
    def steepestAscent (self,n,tol=1e-8,maxit=1000,maxtry=25):
        ...
        for i in xrange(repeat):
            ...
            while tolMet==False:
                ...
                oldEval=g1
                oldPoint=X
                g1,g2=self.Vn(X[0,0:self._n1].reshape((1,self._n1)),self._X,n,self._y)
    ...
    def graphAn (self,n):
        ...
        for i in xrange(m):
            ...
            VOI[i]=self.Vn(points[i],self._X[0:n-1,:],n,self._y[0:n-1,:],False)
            variance[i]=self.varN(points[i],n)
            
        plt.plot(points,VOI)
        plt.xlabel('x',fontsize=26)
        plt.ylabel('VOI',fontsize=24)
        plt.savefig('%d'%n+"VoiStandardGaussian.pdf")
     #   plt.title('VOI for n=%d'%n)
      ...
      #  plt.title('$\mu_%d(x)$'%n+' vs G(x)')
        plt.savefig('%d'%n+"StandardGaussian.pdf")
        plt.close("all")
    ...
    def getNPoints(self,n,i1,tol=1e-8,maxit=1000,maxtry=25):
        if self._old==False:
            ...
            f=open('%d'%i1+"G(xn)KGAnalytic.txt","w")
            self._n=self._n+self._mPrior
            for i in xrange(1,n+1):
                print i
                x1,y1,n1,maxVn=self.steepestAscent(i+self._mPrior)
                x21,val=self.SteepestAscent2(self._X[i+self._mPrior-1,:],i+self._mPrior)
                np.savetxt(f,-x21**2)
    ...
    def SteepestAscent2 (self,x,n,tol=1e-8,maxit=1000,maxtry=25):
  #      n=self._n
        A=self.An(n,self._X)
        L=np.linalg.cholesky(A)
