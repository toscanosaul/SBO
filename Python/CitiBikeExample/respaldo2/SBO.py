...
class SBO:
    ...
    def steepestAscent (self,n,tol=1e-8,maxit=1000,maxtry=25):
      ...
      #  iteration(c,d,n1,n2,Xobs,W,n,y,Vn,goldenSectionLineSearch)
        ...
        n2=self._n2
        y2=self._y
        results_async = [pool.apply_async(iteration,args=(c2,d2,n1,n2,X2,W2,n,y2,self.Vn,self.goldenSectionLineSearch,)) for i in range(repeat)]
        output = [p.get() for p in results_async]
        print output
        
        for i in range(repeat):
            val[i,0]=output[i][0]
            val[i,1:self._n1+1]=listTemp[i][1:self._n1+1]
        print val
        MAX=np.argmax(val[:,0])
 ...
 #       muN=self._muStart+np.dot(temp2.T,temp1)
   ...
   #     a,b,gamma,BN,L,B=self.update(self._points,n,self._y,self._X,self._W,xNew,wNew)
        ...
        plt.legend()
  #      plt.title("Graph of $a_%d(x)$"%n+" vs G(x)")
        plt.xlabel('x',fontsize=26)
        ...
        if self._old==False:
          ...
          #  m=100
           ...
           # x=np.mean(self._Xprior,axis=0)
            ...
            f=open("G(xn)SBOAnalytic.txt","w")
            np.savetxt(f,np.array([self._muStart]))
            for i in xrange(1,n+1):
                print i
                x1,w1,y1,n1,maxVn=self.steepestAscent(i)
                ##checar multidimensional
                x21,val=self.SteepestAscent2(self._X[i-1,:])
                print x21
                np.savetxt(f,-x21**2)
    ...
    def SteepestAscent2 (self,x,tol=1e-8,maxit=1000,maxtry=25):
        ...
        def iteration(i):
            ...
            while tolMet==False:
                ...
                if max(np.reshape(abs(x-oldPoint),x.shape[1]))<tol or iter > maxit:
                    if (any(x[0,:]<self._c)):
                        index2=np.where(x[0,:]<self._c)
                        x[0,index2]=self._c[index2]
                    if (any(x[0,:]>self._d)):
                        index2=np.where(x[0,:]>self._d)
                        x[0,index2]=self._d[index2]
                    g1=self.aN_grad(x,L,y2,False)
                    val[i,0]=g1
                    val[i,1:]=X
                    tolMet=True
        ...
        pool=multiprocessing.Pool()
        results_async= [pool.apply_async(iteration) for i in range(repeat)]        
        MAX=np.argmax(val[:,0])
...
def iteration(c,d,n1,n2,Xobs,W,n,y,Vn,goldenSectionLineSearch):
    val=np.zeros(n1+1)
    tolMet=False
    iter=0
    ...
    while tolMet==False:
        ...
        if (abs(X)[0,0]>3):
            g1=Vn(X[0,0:n1].reshape((1,n1)),X[0,n1:n2+n1].reshape((1,n2)),Xobs,W,n,y,False)
            val[i,0]=g1
            val[i,1:]=X
            tolMet=True
            break
            
        g1,g2=Vn(X[0,0:n1].reshape((1,n1)),X[0,n1:n2+n1].reshape((1,n2)),Xobs,W,n,y)
        while g1==0:
            if iter==1:
              #  print n
                x=np.array(np.random.uniform(c,d)).reshape((1,n1))
                w1=np.array(np.random.uniform(c,d)).reshape((1,n2))
                X=np.concatenate((x,w1),1)
                g1,g2=self.Vn(X[0,0:n1].reshape((1,n1)),X[0,n1:n2+n1].reshape((1,n2)),Xobs,W,n,y)
            else:
               # print n
                val[i,0]=oldEval
                val[i,1:]=oldPoint
        ...
        def fns(alpha,X_=oldPoint,g2=g2):
            ...
            x_=tmp[0,0:n1]
            w_=tmp[0,n1:n2+n1]
            return Vn(x_,w_,Xobs,W,n,y,grad=False)
