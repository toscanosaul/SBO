...
from VOI_SBO_MATERN import Vn,aN_grad
...
#alpha1,alpha2,sigma0 are the parameters of the covariance matrix of the gaussian process (alpha=1/l^2, l is the length scale)
...
class SBO:
    def __init__(self,f,alpha1,alpha2,sigma0,lamb,muStart,n1,n2,l,c,d,wStart,M,h,h2,A,functionY,mPrior=2,old=False,Xold=-1,Wold=-1,yold=-1):
        self._k=GPy.kern.Matern32(input_dim=n1+n2, variance=sigma0**2, lengthscale=np.concatenate(((1.0/np.array(alpha1))**(.5),(1.0/np.array(alpha2))**(.5))),ARD=True)
   ...
   #     a,b,gamma,BN,L,B=self.update(self._points,n,self._y,self._X,self._W,xNew,wNew)
    ...
    def getNPoints(self,n,tol=1e-8,maxit=1000,maxtry=25):
        ...
        #    print "ya"
            ...
            n1=self._n1
            n2=self._n2
            alpha1=np.array(0.5/(np.array(m.Mat32.lengthscale[0:n1]))**2)
            alpha2=np.array(0.5/(np.array(m.Mat32.lengthscale[n1:n1+n2]))**2)
            variance0=np.array(m.Mat32.variance[0])
            muStart=np.array(m.Gaussian_noise.variance[0])
            self._alpha1=alpha1
            self._alpha2=alpha2
            ...
            f5=open("oldConditionalVariancepoisson3.txt",'w')
            f.close()
            g.close()
            f2.close()
            f3.close()
          ...
          #  f.close()
            self._B=np.loadtxt("oldBpoisson3.txt")
            for i in xrange(1,n+1):
                ...
                with open("samplesVSvaluePointsPoisson3.txt", "a") as f4:
                    np.savetxt(f4,x3)
                f4.close()
                with open("oldConditionalVariancepoisson3.txt", "a") as f4:
                    np.savetxt(f4,self._varianceObservations)
                f4.close()
                with open("oldYpoisson3.txt.txt", "a") as f:
                    np.savetxt(f,self._y)
                f.close()
    ...
    def optimize(self,n):
        self.getNPoints(n)
        x,val=SteepestAscent_aN (self._n+self._M,self._n1,self._n2,self._X,self._W,aN_grad,self._c,self._d,self._y,self._muStart,self._variance0,self._alpha1,self._alpha2,self._lambda,self._k)
        return x,val
    

        
    def getPoints(self):
        A=self.An(self._n,self._X,self._W,self._varianceObservations)
