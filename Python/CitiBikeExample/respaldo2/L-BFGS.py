...
class LBFGS:
    def __init__(self,m,f,gradF,dimension):
    ...
    def lineSearch(self,g,dg,grad,c1=0.25,c2=0.75,alphaMax=0.01):
        alpha=np.zeros(2)
        alpha[0]=0.0
    ...
    def BFGS(self,xStart,tol=10e-5):
        k=0
        conv=False
        ...
        while conv==False:
            ...
            def g(t):
                return self.f(self.points[k%m,:]+t*self.gradient[k%m,:])
            def dg(t):
                return self.gradF(self.points[k%m,:]+t*self.gradient[k%m,:])
            alpha=self.lineSearch(g,dg,self.gradient[k%m,:])
            ...
            k=k+1
            if (np.sqrt(np.sum(self.gradient[(k)%m,:]))<tol):
                conv=True
