...
sys.path.append("..")
import numpy as np
from simulationPoissonProcesNonHomogeneousDAY import *
from math import *
...
def noisyF(XW,n):
    ...
    for i in xrange(n):
        simulations[i]=g(TimeHours,w,x,nSets,
                         data,cluster,bikeData,poissonParameters,nDays,
			 Avertices,poissonArray,exponentialTimes,day)
...
def simulatorW(n,ind=False):
    ...
    for i in range(n):
	    wPrior[i,0]=np.random.poisson(poissonParameters[indexes[i]],1)
            wPrior[i,1]=indexes[i]
    if ind:
...
misc=inter.Miscellaneous(randomSeed,parallel,nF=numberSamplesForF,tP=trainingPoints,prefix="TestFinalNonHomogeneous3Day")
