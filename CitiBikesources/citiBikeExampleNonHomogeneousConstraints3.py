...
upperNumberTrips=4000
indInfSource1=np.where(poissonParameters<upperNumberTrips)[0]
indInfSource2=np.where(poissonParameters>=upperNumberTrips)[0]


def simulatorW1(n,ind=False):
    """Simulate n vectors w
    ...
    """
    wPrior=np.zeros((n,n2))
    nSup=len(indInfSource1)
    indexes=np.random.randint(0,nSup,n)
    for i in range(n):
	for j in range(n2):
	    wPrior[i,j]=np.random.poisson(poissonParameters[indInfSource1[indexes[i]]],1)
    indCam=np.where(wPrior[:,0]>=upperNumberTrips)[0]
    if len(indCam)>0:
	wPrior[indCam,0]=upperNumberTrips-1
    if ind:
	return wPrior,indexes
    else:
	return wPrior

def simulatorW2(n,ind=False):
    """Simulate n vectors w
    ...
    """
    wPrior=np.zeros((n,n2))
    nSup=len(indInfSource2)
    indexes=np.random.randint(0,nSup,n)
    for i in range(n):
	for j in range(n2):
	    wPrior[i,j]=np.random.poisson(poissonParameters[indInfSource2[indexes[i]]],1)
	    
    indCam=np.where(wPrior[:,0]<upperNumberTrips)[0]
    if len(indCam)>0:
	wPrior[indCam,0]=upperNumberTrips
    if ind:
	return wPrior,indexes
    else:
	return wPrior
...
listW=[simulatorW1,simulatorW2]
Objective=inter.objective(g,n1,noisyF,numberSamplesForF,sampleFromXVn,
                          listW,estimationObjective,numberSources,sampleFromXAn)
...
#trainInf=[train1,train2]
misc=inter.Miscellaneous(randomSeed,parallel,nF=numberSamplesForF,tP=trainingPoints,prefix="InfSources112715")
...
train2=int(trainingPoints*.75)
train1=trainingPoints-train2

Wtrain1=simulatorW1(train1)


Wtrain2=simulatorW2(train2)


XWtrain=[]

XWtrain.append(np.concatenate((Xtrain[0:train1,:],Wtrain1),1))
XWtrain.append(np.concatenate((Xtrain[train1:trainingPoints,:],Wtrain2),1))


dataObj=inter.data(XWtrain,yHist=None,varHist=None)

trainInf=[train1,train2]
dataObj.getTrainingDataSBO(trainInf,noisyF,numberSamplesForF,parallel)
...
probsTemp1=[]
probsTemp1.append(probsTemp[0:upperNumberTrips-L]/probInf1)

probInf2=1.0-probInf1
...
def B(x,XW,n1,n2,kernel,infSource,logproductExpectations=None):
    ...
    if logproductExpectations is None:
        ...
        for j in xrange(n2):
	    temp=expectation(W[j],alpha2[j],poissonParameters,nDays,probsTemp1[infSource],infSource)
...
def computeLogProductExpectationsForAn(W,N,kernel,infSource):
  ...
  #  parameterLamb=parameterSetsPoisson
    ...
    for i in xrange(N):
        ...
        for j in xrange(n2):
	    temp=expectation(W[i,j],alpha2[j],poissonParameters,nDays,probsTemp1[infSource],infSource)
            logproductExpectations[i]+=np.log(temp)
    return logproductExpectations

def combineInfSources(a,b):
    probInf2=1.0-probInf1
    return probInf1*a+probInf2*b

stat=stat.SBOGP(B=B,dimNoiseW=n2,dimPoints=n1,trainingData=dataObj,
                dimKernel=n1+n2, numberTraining=trainInf,
                computeLogProductExpectationsForAn=
                computeLogProductExpectationsForAn,scaledAlpha=scaleAlpha,
		numberInf=numberSources,combineInfSources=combineInfSources)
...
def expectation2(z,alpha,parLambda,nDays,probs,infSource):
    if infSource==0:
	L=650
	M=upperNumberTrips
    else:
...
def gradWB(new,kern,BN,keep,points,infSource):
   ...
   # parameterLamb=parameterSetsPoisson
    ...
    for i in xrange(n2):
        ...
        del a[i]
        for r in a:
	    temp=expectation(W[r],alpha2[r],poissonParameters,nDays,probsTemp1[infSource],infSource)
            logproductExpectations+=np.log(temp)

	temp=expectation2(W[i],alpha2[i],poissonParameters,nDays,probsTemp1[infSource],infSource)
    ...
    """ Evaluates a_{i} and its derivative, which is the expectation of the GP on g(x).
        ...
        Args:
            ...
            L: List with Cholesky decomposition of the matrix A, where A is the covariance
               matrix of the past obsevations (x,w).
            dataObj: List with Data object.
            stat: Statistical object.
            onlyGradient: True if we only want to compute the gradient; False otherwise.
            logproductExpectations: List Vector with the logarithm of the product of the
                                    expectations of np.exp(-alpha2[j]*((z-W[i,j])**2))
                                    where W[i,:] is a point in the history.
    """
