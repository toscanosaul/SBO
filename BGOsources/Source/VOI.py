...
class VOISBO(VOI):
    ...
    def aANDb(self,n,x,xNew,wNew,L,temp2,past,kernel,B,infSource):
        """
        Output: A tuple with:
        ...
        x=np.array(x)
        m=x.shape[0]
        tempN=self._numberTraining[infSource]+n
        BN=np.zeros([m,1])
        n2=self.n2
        BN[:,0]=B(x,np.concatenate((xNew,wNew)),self.n1,n2,kernel,infSource) #B(x,n+1)
        n1=self.n1
        n2=self.n2
    ...
    def evalVOI(self,n,pointNew,a,b,c,keep,keep1,M,gamma,BN,L,inv,aux4,kern,XW,infSource,
        ...
        nTraining=self._numberTraining[infSource]
        tempN=nTraining+n
        ...
        gradWB=self._gradWBfunc(pointNew,kern,BN,keep,self._points,infSource)
    ...
    def VOIfunc(self,n,pointNew,grad,L,temp2,a,scratch,kern,XW,B,infSource,onlyGradient=False):
        """
        Output:
            Evaluates the VOI and it can compute its derivative. It evaluates
            the VOI, when grad and onlyGradient are False; it evaluates the
        ...
        b,gamma,BN,temp1,aux4=self.aANDb(n,self._points,pointNew[0,0:n1],pointNew[0,n1:n1+self.n2],L,
                                    temp2=temp2,past=XW,kernel=kern,B=B,infSource=infSource)
        ...
        if onlyGradient:
            return self.evalVOI(n,pointNew,a,b,c,keep,keep1,M,gamma,BN,L,scratch=scratch1,
                                ...
                                kern=kern,XW=XW,infSource=infSource)
        if grad==False:
