...
def AffineBreakPointsPrep(a, b):
    ...
    cleft = (a.astype(np.float) - a1) / (
    b1 - b)  # instersection with leftmost line
    cright = (a.astype(np.float) - a3) / (
    b3 - b)  # instersection with rightmost line
    ...
    keep = [i for i in xrange(len(b)) if
            b[i] == b1 or b[i] == b3 or cleft[i] <= c2left or cright[
                i] >= c2right]
