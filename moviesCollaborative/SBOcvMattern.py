...
def B(x,XW,n1,n2,kernel,logproductExpectations=None):
    ...
    inda=n1+n2
    W=XW[n1:inda]
    alpha2=((kernel.alpha[n1:n1+n2])**2)/scaleAlpha[n1:n1+n2]**2
	...
	a=expectation(x[i,:],X,W[0],alpha1,alpha2)
...
def gradWB(new,kern,BN,keep,points):
    ...
    """
    alpha1=((kern.alpha[0:n1])**2)/scaleAlpha[0:n1]**2
    alpha2=((kern.alpha[n1:n1+n2])**2)/scaleAlpha[n1:n1+n2]**2
	...
	sum1=0
        ...
        for j in range(n1):
            ...
            sum1=sum1+(5.0/3.0)*(np.exp(-np.sqrt(5*r)))*(-1.0-np.sqrt(5*r))\
                 *(alpha2*(j-W))
        gradWBarray[j,0]=sum1*(variance0)/n1
    return gradWBarray
