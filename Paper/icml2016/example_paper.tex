...
In our next examples, we demonstrate that SBO should be used when $\mbox{corr}\left[f\left(x,\w,\z\right),f\left(x,\w',\z'\right)|f,x,w,w'\right]$ is large enough.
...
\begin{align*}
\mbox{corr}\left[f\left(x,\w,\z\right),f\left(x,\w',\z'\right)|f,x,w,w'\right]=\frac{1}{jn-1}\mbox{exp}\left(-\beta\left\Vert \left(x,w\right)-\left(x,w'\right)\right\Vert _{2}^{2}\right),
\end{align*}
...
In conclusion, a large correlation $\mbox{corr}\left[f\left(x,\w,\z\right),f\left(x,\w',\z'\right)|f,x,w,w'\right]$ is fundamental, for SBO outperforms KG. A large variance reduction is helpful to improve SBO results, but it is not sufficient.
