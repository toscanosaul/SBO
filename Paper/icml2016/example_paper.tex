...
In our next examples, we demonstrate that SBO should be used when the correlation $\mbox{corr}\left[f\left(x,w,z\right),f\left(x,w'z\right)|x,w\right]$ is large enough. We also show that the variance reduction $\frac{\mbox{Var}\left(f\left(x,w,z\right)|x\right)-\mbox{Var}\left(f\left(x,w,z\right)|x,w\right)}{\mbox{Var}\left(f\left(x,w,z\right)|x\right)}$ benefits SBO.
